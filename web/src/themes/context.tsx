...
import { BUILTIN_THEMES, defaultTheme, stellaTheme } from "./presets";
import {
  FONT_CHOICES,
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [themeName, setThemeName] = useState<string>(() => {
    if (typeof window === "undefined") return "stella";
    const stored = window.localStorage.getItem(STORAGE_KEY) ?? "stella";
  ...
  const setTheme = useCallback(
    (name: string) => {
      ...
      const next = knownNames.has(name) ? name : "stella";
      setThemeName(next);
      if (typeof window !== "undefined") {
