...
const APP_NAME = PACKAGE_JSON.build?.executableName || PACKAGE_JSON.build?.productName || 'Stella'
...
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, `${APP_NAME}.app`)
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', APP_NAME),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, `${APP_NAME}.exe`),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, APP_NAME),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `${APP_NAME}-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `${APP_NAME}-${PACKAGE_JSON.version}`
  const candidates = fs
...
function resolveNsisPath() {
  // electron-builder uses the product's artifactName template from package.json.
